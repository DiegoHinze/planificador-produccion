...
let lotes = JSON.parse(localStorage.getItem('lotes') || '[]'); // [{id, obraId, cantidades, fechaLimite, agregadoEl}]
...
function saldoPorTipo(obraId) {
  // Calcula cuánto ya fue planificado en lotes anteriores por tipo
  const planificado = {};
...
function abrirModalLote(obraId) {
  ...
  document.getElementById('lote-aberturas').innerHTML = obraLoteActual.aberturas.map(a=>{
    ...
    return `<div class="abertura-row" style="${completa?'opacity:0.45':''}">
      ...
      <td style="text-align:center">
        ...
        <div style="font-size:10px;color:#9b9b98">de ${a.total} ${yaPlani>0?`(${yaPlani} ya en lotes)`:''}</div>
      </td>
      ${completa
...
function guardarLote() {
  ...
  obraLoteActual.aberturas.forEach(a=>{
    const v=parseInt(document.getElementById('lote-qty-'+a.tipo)?.value)||0;
...
// ============================================================
// RENDER LOTES (pantalla planificar)
// ============================================================
function renderLotes() {
  ...
  container.innerHTML=Object.entries(porObra).map(([obraId,lotesObra])=>{
    ...
    return `<div class="obra-plan-card" style="margin-bottom:16px">
      <div class="obra-plan-header">
        ...
        <button class="btn-secondary" style="font-size:12px;padding:5px 12px" onclick="abrirModalLote('${obraId}')">+ Agregar lote</button>
      </div>
      <div style="padding:12px 16px">
        ${lotesObra.map((l,i)=>{
          const total=Object.values(l.cantidades).reduce((s,v)=>s+v.qty,0);
          const fl=l.fechaLimite?new Date(l.fechaLimite).toLocaleDateString('es-UY',{day:'numeric',month:'short',year:'numeric'}):'Sin fecha';
          return `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:0.5px solid #e5e5e2;flex-wrap:wrap;gap:8px">
            <div>
              <span style="font-size:12px;font-weight:600;color:#1a1a18">Lote ${i+1}</span>
              <span style="font-size:12px;color:#6b6b68;margin-left:8px">${total} aberturas</span>
              <span style="font-size:11px;color:#9b9b98;margin-left:8px">· ${Object.keys(l.cantidades).map(t=>`${t}×${l.cantidades[t].qty}`).join(', ')}</span>
            </div>
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-size:12px;color:#6b6b68">📅 ${fl}</span>
              <button class="del-row-btn" onclick="eliminarLote('${l.id}')" style="font-size:14px">✕</button>
            </div>
          </div>`;
        }).join('')}
      </div>
  ...
  if(obrasSinLote.length){
    container.innerHTML+=`<div style="margin-top:8px">
      <div style="font-size:12px;color:#6b6b68;margin-bottom:8px">Obras sin lotes:</div>
...
function renderOperariosConfig() {
  ...
  <div style="margin-bottom:16px">
    ...
    <div id="prep-operarios">
      ${prepNombres.map((n,i)=>`<div class="operario-row" id="prep-row-${i}">
  ...
  <div>
    ...
    <div id="arm-operarios">
      ${opNames.map((n,i)=>`<div class="operario-row" id="arm-row-${i}">
...
function generarPlan() {
  ...
  if (!lotes.length){alert('Agregá al menos un lote de producción.');return;}
  localStorage.setItem('fecha-inicio',fi);

  const nArm = opNames.length;
  const nPrep = prepNombres.length;

  // Construir pedido global a partir de todos los lotes
  let pedidoGlobal = [];
  lotes.forEach(lote=>{
    Object.entries(lote.cantidades).forEach(([tipo,v])=>{
      ...
      pedidoGlobal.push({
        loteId:lote.id,
        obraId:lote.obraId,
        obraNombre:lote.obraNombre,
        tipo,
        cat:v.cat||abRef?.cat||'corrediza',
        qty:v.qty,
        restante:v.qty,
  ...
  while(hayPendiente && semIdx<52){
    ...
    for(let di=0;di<5;di++){
      ...
      // ---- CORTE ----
      // El cortador corta siempre que haya pendiente
      // También arranca anticipado para el siguiente lote si el stock está bajo
      const corteCap = esV ? c.corteV : c.corteLJ;
      const prepCapHoy = (esV ? c.prepV : c.prepLJ) * nPrep;
      const diasStockRestante = stockCortado / Math.max(prepCapHoy,1);
      const cortadoHoy = pendTotal>0.01 ? corteCap : 0;
      stockCortado += cortadoHoy;

      // ---- PREPARACION ----
      // Preparadores base + armadores libres rotan automáticamente
      const bufTotalAntes = bufferPrep.reduce((s,b)=>s+b.qty,0);
      const capArmTotal = nArm * (c.armCorr);
      // Armadores libres: los que no tienen buffer suficiente
      const armLibres = bufTotalAntes < capArmTotal * 0.3 
        ? Math.min(nArm, Math.ceil((capArmTotal*0.3 - bufTotalAntes) / c.armPrepLJ))
        : 0;
      const prepExtra = armLibres * (esV ? c.armPrepV : c.armPrepLJ);
      const capPrepTotal = prepCapHoy + prepExtra;

      const dispPrep = Math.min(capPrepTotal, pendTotal, stockCortado);

      // Distribuir preparación proporcionalmente entre obras/lotes
      let prepHoy = [];
      ...
      if(totalPend>0.01 && restPrep>0.01){
        Object.entries(obrasPend).forEach(([obraId,total])=>{
          ...
          pendiente.filter(a=>a.obraId===obraId&&a.restante>0.01).forEach(a=>{
            ...
            if(q>0.01){prepHoy.push({tipo:a.tipo,cat:a.cat,qty:q,obraId:a.obraId,obraNombre:a.obraNombre,loteId:a.loteId});cuota-=q;}
          });
        });
        // Sobrante
        ...
        if(sob>0.01){
          pendiente.filter(a=>a.restante>0.01).forEach(a=>{
            ...
            const ya=ex?ex.qty:0;
            const q=Math.min(a.restante-ya,sob);
      ...
      // Distribuir buffer entre armadores respetando cap diaria exacta
      const bufDisp=[...bufferPrep.filter(b=>b.qty>0.01)];
      bufDisp.forEach(b=>{
        let rest=b.qty;
        while(rest>0.01){
      ...
      // Descontar armado del buffer
      armHoy.forEach(a=>{
        const bx=bufferPrep.find(b=>b.tipo===a.tipo&&b.obraId===a.obraId);
        if(bx) bx.qty=Math.max(0,bx.qty-a.qty);
      });
      bufferPrep=bufferPrep.filter(b=>b.qty>0.01);

      // Armadores libres → rotan a prep (mostrar en plan)
      opCargaFrac.forEach((f,i)=>{ if(f<0.05&&hayTrabajo) armRotadosHoy.push(i); });

      // Estado de cada operario en el día
      const opEstado=opNames.map((nombre,i)=>{
        ...
        if(armRotadosHoy.includes(i)) return{nombre,estado:'prep',tareas:[],pct:0};
      ...
      pedidoGlobal.forEach(a=>{
        if(!completadoEn[a.obraId]){
          if((pendAhora[a.obraId]||0)<0.01&&(bufAhora[a.obraId]||0)<0.01)
    ...
    const pct=Math.min(100,capBaseSem>0?Math.round((prepSemTotal/capBaseSem)*100):0);
  ...
  obraIds.forEach(obraId=>{
    const lotesObra=lotes.filter(l=>l.obraId===obraId);
    const termina=completadoEn[obraId];
    const obraRef=obras.find(o=>o.id===obraId);
    if(!termina||!obraRef) return;
    const fl=lotesObra.find(l=>l.fechaLimite)?.fechaLimite;
    if(fl){
      const fechaLim=new Date(fl+'T23:59:59');
      if(termina>fechaLim){
        const dias=Math.ceil((termina-fechaLim)/(1000*60*60*24));
        alertas+=`<div class="alert alert-danger" style="margin-bottom:8px">
          <strong>⚠ ${obraRef.nombre}:</strong> No se puede cumplir la fecha del ${fmtF(fechaLim)}.
          Terminaría el <strong>${fmtFull(termina)}</strong> (${dias} día(s) de atraso).
          <div style="margin-top:4px;font-size:12px">Considerá agregar un preparador en Configuración para acelerar la producción.</div>
        </div>`;
      } else {
        const dias=Math.ceil((fechaLim-termina)/(1000*60*60*24));
        alertas+=`<div class="alert alert-ok" style="margin-bottom:8px">
          <strong>✓ ${obraRef.nombre}:</strong> Se completa el <strong>${fmtFull(termina)}</strong>, ${dias} día(s) antes del límite.
        </div>`;
      }
    } else {
      alertas+=`<div class="alert alert-info" style="margin-bottom:8px">
        <strong>${obraRef.nombre}:</strong> Se completa el <strong>${fmtFull(termina)}</strong>.
      </div>`;
    }
  });
  ...
  document.getElementById('plan-metrics').innerHTML=`
    ${obraIds.map(id=>{
      ...
      const fl=lotes.find(l=>l.obraId===id&&l.fechaLimite)?.fechaLimite;
      const ok=fl&&termina?new Date(fl+'T23:59:59')>=termina:true;
      return `<div class="metric"><div class="metric-label">${o.nombre}</div><div class="metric-value">${total}</div>
        <div class="metric-sub" style="color:${ok?'#27500a':'#791f1f'}">${termina?fmtF(termina):'—'} ${fl?(ok?'✓':'⚠'):''}</div></div>`;
    }).join('')}
    <div class="metric"><div class="metric-label">Total global</div><div class="metric-value">${totalGlobal}</div><div class="metric-sub">aberturas en ${lotes.length} lotes</div></div>
...
function renderSemana(sem){
  ...
  const diasHtml=diasData.map(d=>{
    ...
    const armChips=d.opEstado.map((op,i)=>{
      if(op.estado==='libre') return`<div style="font-size:11px;color:#9b9b98;margin-bottom:2px">${op.nombre}: <strong>libre</strong></div>`;
      ...
      return`<div style="display:flex;align-items:center;gap:3px;flex-wrap:wrap;margin-bottom:2px">
        <span style="font-size:10px;color:#9b9b98;min-width:72px">${op.nombre} (${op.pct}%)</span>
        ${op.tareas.map(t=>`<span class="chip chip-op${Math.min(i+1,4)}">${t.obraNombre.split(' ')[0]} ${t.tipo}×${Math.round(t.qty)}</span>`).join('')}
      </div>`;
    }).join('');
  ...
  const libreHtml=pendAlFinal<0.1?`
    <div class="free-slot">
      ...
      <div class="free-slot-sub">Podés agregar nuevos lotes desde la pantalla de planificación para continuar el plan.</div>
    </div>`:'';
