...
let ordenObras = JSON.parse(localStorage.getItem('orden-obras') || '[]');
...
const OBRA_DEMO = {
  ...
  sistema: 'Nordical 90° Corrediza + Fijo y HA 62', fechaEntrega: '2026-09-30',
  aberturas: [
...
function saveOrden()     { localStorage.setItem('orden-obras', JSON.stringify(ordenObras)); }
...
function renderObras() {
  ...
  grid.innerHTML = obras.map(o => {
    const entrega = o.fechaEntrega ? new Date(o.fechaEntrega).toLocaleDateString('es-UY',{day:'numeric',month:'short',year:'numeric'}) : 'Sin fecha';
    return `<div class="obra-card">
      ...
      <div class="obra-stats">
        ...
        <div class="obra-stat">📅 <strong>${entrega}</strong></div>
      </div>
      <div class="obra-actions">
        <button class="btn-primary" style="font-size:12px;padding:5px 12px" onclick="irAPlanificar('${o.id}')">Planificar</button>
...
function irAPlanificar(obraId) {
  showSection('planificar');
  // Scroll a la obra en el plan global
  setTimeout(() => {
    const el = document.getElementById('obra-plan-'+obraId);
...
function eliminarObra(id) {
  ...
  delete cantidadesGlobal[id];
  saveObras(); saveCantidades();
...
function guardarObra() {
  ...
  const fechaEntrega = document.getElementById('obra-fecha-entrega').value;
  ...
  obras.push({id:'obra-'+Date.now(),nombre,ppto,cliente,sistema,fechaEntrega,aberturas});
...
function renderPlanGlobal() {
  ...
  // Ordenar obras por fecha de entrega
  ...
  container.innerHTML = obrasOrdenadas.map((o,idx) => {
    const cant = cantidadesGlobal[o.id] || {};
    const totalSel = Object.values(cant).reduce((s,v)=>s+v,0);
    const entrega = o.fechaEntrega ? new Date(o.fechaEntrega).toLocaleDateString('es-UY',{day:'numeric',month:'short',year:'numeric'}) : 'Sin fecha';
    const urgencia = getUrgencia(o.fechaEntrega);

    return `<div class="obra-plan-card" id="obra-plan-${o.id}">
      <div class="obra-plan-header">
        <div>
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
            <span class="prioridad-badge">#${idx+1}</span>
            <span class="obra-plan-nombre">${o.nombre}</span>
            <span class="wbadge ${urgencia.cls}">${urgencia.txt}</span>
          </div>
          <div style="font-size:12px;color:#6b6b68">Ppto. ${o.ppto} · ${o.cliente} · Entrega: <strong>${entrega}</strong></div>
        </div>
        <div style="font-size:13px;font-weight:600;color:${totalSel>0?'#27500a':'#9b9b98'}">${totalSel>0?totalSel+' seleccionadas':'Sin selección'}</div>
      </div>
        ...
        ${o.aberturas.map(a=>`
          <div class="abertura-row">
            ...
            <input type="number" min="0" max="${a.total}" value="${cant[a.tipo]||0}" class="qty-input"
              onchange="setCantidad('${o.id}','${a.tipo}',parseInt(this.value)||0)">
          </div>`).join('')}
      </div>
    </div>`;
  }).join('');
...
function setCantidad(obraId, tipo, val) {
  ...
  const header = document.querySelector(`#obra-plan-${obraId} .obra-plan-header div:last-child`);
  if (header) header.innerHTML = `<span style="font-size:13px;font-weight:600;color:${totalSel>0?'#27500a':'#9b9b98'}">${totalSel>0?totalSel+' seleccionadas':'Sin selección'}</span>`;
}

function getUrgencia(fechaEntrega) {
  if (!fechaEntrega) return { cls:'wb-warn', txt:'Sin fecha' };
  const dias = Math.ceil((new Date(fechaEntrega)-new Date())/(1000*60*60*24));
  if (dias<30)  return { cls:'wb-crit', txt:'Urgente' };
  if (dias<60)  return { cls:'wb-warn', txt:'Próxima' };
  return { cls:'wb-ok', txt:'En plazo' };
}

function limpiarPlanGlobal() {
  cantidadesGlobal={};
  rotacionesAprobadas={};
  saveCantidades(); saveRotaciones();
  renderPlanGlobal();
...
// ============================================================
// LOGICA DE PLANIFICACION GLOBAL
// ============================================================
...
function generarPlan() {
  ...
  // Construir pedido global ordenado por fecha de entrega
  const obrasConPedido = obras
    .filter(o => {
      const cant = cantidadesGlobal[o.id]||{};
  ...
  // Armar lista global de aberturas con referencia a obra, manteniendo prioridad
  let pedidoGlobal = [];
  ...
  const resumenObras = obrasConPedido.map(o=>{
    ...
    const entrega=o.fechaEntrega?new Date(o.fechaEntrega).toLocaleDateString('es-UY',{day:'numeric',month:'short',year:'numeric'}):'Sin fecha';
    return `<div class="metric"><div class="metric-label">${o.nombre}</div><div class="metric-value">${total}</div><div class="metric-sub">Entrega: ${entrega}</div></div>`;
  }).join('');
  ...
  document.getElementById('plan-metrics').innerHTML=`
    ...
    <div class="metric"><div class="metric-label">Cap. prep./sem.</div><div class="metric-value">${capBaseSem}</div><div class="metric-sub">base sin rotaciones</div></div>
    <div class="metric"><div class="metric-label">Semanas mínimas</div><div class="metric-value">${Math.ceil(totalGlobal/capBaseSem)}</div><div class="metric-sub">estimado</div></div>`;
  ...
  while(pendiente.some(a=>a.restante>0)&&semIdx<26){
    ...
    for(let di=0;di<5;di++){
      ...
      // Preparar respetando prioridad de obra (orden ya viene ordenado)
      let prepHoy=[],restPrep=dispPrep;
      for(let a of pendiente){
        if(restPrep<=0) break;
        const q=Math.min(a.restante,restPrep);
        if(q>0){prepHoy.push({tipo:a.tipo,cat:a.cat,qty:q,obraId:a.obraId,obraNombre:a.obraNombre});restPrep-=q;}
      }
      ...
      for(let p of prepHoy){
        ...
        while(rest>0){
          ...
          opCarga[mi]+=asig/capOp; rest-=asig;
        }
      }
  ...
  if(!document.getElementById('btn-imprimir')){
    ...
    document.getElementById('plan-metrics').parentNode.insertBefore(btnRow,document.getElementById('plan-metrics'));
  }
...
// ============================================================
// Colores por obra
const OBRA_COLORS = ['#0c447c','#27500a','#633806','#72243e','#3c3489','#5a3e00'];
function obraColor(obraId){
  const idx=obras.findIndex(o=>o.id===obraId)%OBRA_COLORS.length;
  return OBRA_COLORS[Math.max(0,idx)];
}
...
function renderSemana(sem){
  ...
  if(!rotActiva&&pendAlFinal>0){
    ...
    if(optCorte){
      opts.push({
        ...
        detalle:`Después de ${optCorte.dia+1} día(s) de corte habrá ${optCorte.stock} uds. en stock. Ganancia estimada: +${Math.round(ganCorte)} unidades.`,
        accion:`aplicarRot(${semIdx},{corteAPrep:true,desdeDia:${optCorte.dia+1},armsAPrep:0})`
      });
    } else {
      opts.push({titulo:`${corteNombre} permanece en corte toda la semana`,detalle:`Stock insuficiente para rotar con seguridad.`,accion:null});
    }
    opts.push({
  ...
  const libreHtml=libre>0&&pendAlFinal===0?`
    <div class="free-slot">
      <div class="free-slot-title">✓ Todas las obras completadas — ${libre} espacios disponibles</div>
